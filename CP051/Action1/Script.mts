...
Sub SeleccionarEquipoMovil()
	...
	If (DataTable("e_TipodeAlta", dtLocalSheet) = "Alta Nueva Equipo + Linea") Then
		while JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Nuevo equipo (Para WILSON").JavaButton("Agregar al carrito_2").Exist = false
			wait 1
		Wend
		wait 1
		...
		wait 3
		While ((JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Nuevo equipo (Para WILSON").JavaStaticText("Mostrando 1-6 de 10 equipos(st").Exist) or (JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").JavaStaticText("No existen equipos disponibles").Exist))= false
			wait 1
		Wend
		wait 2
		If JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").JavaStaticText("No existen equipos disponibles").Exist = True Then
		...
		While ((JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").JavaStaticText("No se cuenta con stock").Exist) or (JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Nuevo Plan (Para WILSON").JavaStaticText("Mostrando 1-6 de 20 ofertas").Exist)) = False
			wait 1
		Wend
		wait 1
		If JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").JavaStaticText("No se cuenta con stock").Exist = True Then
			DataTable("s_Resultado",dtLocalSheet) = "Fallido" 
			DataTable("s_Detalle", dtLocalSheet) = JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").JavaStaticText("No se cuenta con stock").GetROProperty("text")
			Reporter.ReportEvent micFail,DataTable("s_Resultado",dtLocalSheet), DataTable("s_Detalle", dtLocalSheet)
			JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").JavaButton("OK").Click
			ExitActionIteration
		End If
...
Sub TipoEnvio()
	...
	Select Case ucase(DataTable("e_MetodoEntrega", dtLocalsheet))
		Case "DELIVERY"'
				wait 2
	...
	If JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Pago Inmediato").JavaList("Tipo de documento:").Exist Then
		...
		For Iterator = 0 To Count-1
			...
			If rs = DataTable("e_MetodoPago",dtLocalSheet) Then
				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Pago Inmediato").JavaList("Medio de pago").Select strDataTable("e_MetodoPago",dtLocalSheet)
				Exit for
			ElseIf Iterator = Count-1 Then
